...
def train_models(draw_df: pd.DataFrame, logger=None):
    ...
    log(f"🔍 Columns after normalization: {df.columns.tolist()}")
    ...
    try:
        ...
        for i, col in enumerate(NUMBER_COLUMNS):
            ...
            model.fit(X, Y[:, i])
            path = os.path.join(MODEL_DIR, MODEL_FILE_NAMES[col])
            joblib.dump(model, path)
            log(f"✅ Trained model for number position {col} → {path}")

        # Train MultiOutput full-set model
        full_model = MultiOutputRegressor(RandomForestRegressor(n_estimators=300, random_state=42))
        full_model.fit(X, Y)
        full_path = os.path.join(MODEL_DIR, MODEL_FILE_NAMES["full"])
        joblib.dump(full_model, full_path)
        log(f"✅ Trained full-set model → {full_path}")

        # Train PowerBall model
        pb_model = RandomForestClassifier(n_estimators=200, random_state=42)
        pb_model.fit(X, Y_pb)
        pb_path = os.path.join(MODEL_DIR, MODEL_FILE_NAMES[POWERBALL_COLUMN])
        joblib.dump(pb_model, pb_path)
        log(f"✅ Trained PowerBall model → {pb_path}")
